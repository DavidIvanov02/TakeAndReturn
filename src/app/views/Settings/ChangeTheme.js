    ...
    _renderItem({ item }) {
        ...
        return (
            <TouchableOpacity onPress={() => setTheme(item.key, this.props.navigation.setParams({ theme: item.color }))}>
                <Text style={[responsive.themeText, { color: item.color }]}>{item.key}</Text>
            </TouchableOpacity>
        );
    }
    /* /Render Item - Render One Row - Item - (Theme)/ */
    ...
    render() {
        ...
        return (
            <Content style={custom.content}>
                ...
                <View style={responsive.themeImageSection}>
                    <Text style={responsive.settingsQuestion}>Изберете вашата тема: </Text>
