            ...
            return (
                <View style={custom.PartView}>
                    <ListItem style={custom.PartListItem}>
                        ...
                        <CheckBox onClick={() => this.handleChange(item.worker_fname, item.worker_sname, item.worker_lname, item.tool_id, item.tool_name, item.tool_count, item.worker_email, item.worker_pincode, item.tool_worker_id, item.date_take, item.hour_take, this.state.date_return, this.state.hour_return)} isChecked={item.checked} checkedCheckBoxColor={'#22364F'} uncheckedCheckBoxColor={'#F5F5F5'} style={custom.FormCheckBox}/>
                    </ListItem>
                </View>
            );
        }
    ...
    render() {
        ...
        if (this.state.isLoading === true) {
            return (
                <Container>
                    <View>
                        <SearchBar ref={(ref) => this.searchBar = ref} data={this.state.data} showOnLoad placeholder="Търсете инструмент в работник" handleChangeText={text => this._handleSearch(text)} backButton={<AntDesign name="arrowleft" style={custom.stackNavigatorSearchArrow} size={24} onPress={() => {this.props.navigation.navigate('ToolsWorkersView')}}/>} backgroundColor={this.props.theme.color} textColor="#F5F5F5" iconColor="#F5F5F5"/>
                    </View>
                    <Content contentContainerStyle={custom.container} style={custom.content}>
