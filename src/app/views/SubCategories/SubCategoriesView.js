...
class SubCategoriesView extends React.PureComponent {
    ...
    _handleRead(subcategory_id, subcategory_name) {
        this.props.navigation.navigate('Tools', { subcategory_id: subcategory_id, subcategory_name: subcategory_name});
    }
    /* /Handle Read Method - Navigate to Tools - Read SubCategory/ */
    ...
    _renderItem = (index, item) => {
        ...
        return (
            <View style={custom.PartView}>
                <ListItem style={custom.PartListItem} onPress={() => this._handleRead(item.subcategory_id, item.subcategory_name)}>
                    <Image style={custom.PartImage} source={require('@app/assets/images/subcategories.png')}/>
                    <Text style={responsive.PartName}> {item.subcategory_name} </Text>
                </ListItem>
            </View>
        );
    };
        ...
        return (
            <View>
                <View style={custom.PartQuickActionButtons}>
                    <AntDesign name="eye" style={custom.PartRead} onPress={() => this._handleRead(item.subcategory_id, item.subcategory_name)}/>
                </View>
                <View style={custom.PartQuickActionButtons}>
        ...
        if (this.state.data.length > 0) {
            return (
                <List style={custom.PartList}>
                    <Text style={custom.categoryTitle}>Категория: {category_name}</Text>
                    <SwipeListView data={this.state.data} renderItem={({index, item}) => this._renderItem(index, item)} renderHiddenItem={({index, item}) => this._renderQuickActionButton(index, item)} leftOpenValue={20 + Math.random() * 150} rightOpenValue={-150} keyExtractor={this._keyExtractor.bind(this)}/>
                </List>
            );
        }
            ...
            return (
                <Content contentContainerStyle={custom.container} style={custom.content}>
                    <Text style={custom.noDataMessage}>
                        Няма добавени подкатегории в категорията:
                    </Text>
                    <Text style={custom.noDataMessage}>
                        {category_name}
                    </Text>
                        ...
                        <Left>
                            <TouchableOpacity hitSlop={{ top: 20, bottom: 20, left: 50, right: 50 }} onPress={() => this.props.navigation.navigate('Categories')}>
                                <Button transparent onPress={() => this.props.navigation.navigate('Categories')}>
                                    <AntDesign name="arrowleft" style={responsive.headerBackIcon} onPress={() => this.props.navigation.navigate('Categories')} />
                                </Button>
                            </TouchableOpacity>
                        </Left>
                        ...
                        <Right>
                            <TouchableOpacity hitSlop={{ top: 20, bottom: 20, left: 50, right: 50 }} onPress={() => this._handleSearch()}>
                                <Button transparent onPress={() => this._handleSearch()}>
                                    <Icon name="search" style={responsive.headerIcon} onPress={() => this._handleSearch()} />
                                </Button>
                            </TouchableOpacity>
                        </Right>
                    </Header>
