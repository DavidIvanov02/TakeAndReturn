...
class CategoriesView extends React.PureComponent {
    ...
    _handleRead(category_id, category_name) {
        this.props.navigation.navigate('SubCategories', { category_id: category_id, category_name: category_name });
    }
    /* /Handle Read Method - Navigate to SubCategories - Read Category/ */
    ...
    _renderItem = (index, item) => {
        ...
        return (
            <View style={custom.PartView}>
                <ListItem style={custom.PartListItem} onPress={() => this._handleRead(item.category_id, item.category_name)}>
                    <Image style={custom.PartImage} source={{ uri: 'asset:/images/' + `${item.category_icon}` }}/>
                    <Text style={responsive.PartName}> {item.category_name} </Text>
                </ListItem>
            </View>
        )
    };
        ...
        return (
            <View>
                <View style={custom.PartQuickActionButtons}>
                    <AntDesign name="eye" style={custom.PartRead} onPress={() => this._handleRead(item.category_id, item.category_name)}/>
                </View>
                <View style={custom.PartQuickActionButtons}>
                        ...
                        <Left>
                            <TouchableOpacity hitSlop={{ top: 20, bottom: 20, left: 50, right: 50 }} onPress={() => this.props.navigation.openDrawer('DrawerOpen')}>
                                <Button transparent onPress={() => this.props.navigation.openDrawer('DrawerOpen')}>
                                    <Icon name="menu" style={responsive.headerIcon} onPress={() => this.props.navigation.openDrawer('DrawerOpen')}/>
                                </Button>
                            </TouchableOpacity>
                        </Left>
